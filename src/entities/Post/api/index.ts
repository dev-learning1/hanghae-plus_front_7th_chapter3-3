...
//
export function fetchPosts(params?: PaginationParams): Promise<PostsResponse> {
  return ky.get(`/api/posts`, { searchParams: makeSearchParams(params) }).json()
}

export function fetchPostsByTag(tag: string, params?: PaginationParams): Promise<PostsResponse> {
  return ky.get(`/api/posts/tag/${tag}`, { searchParams: makeSearchParams(params) }).json()
}

export function fetchPostsSearch(query: string, params?: PaginationParams): Promise<PostsResponse> {
  return ky.get(`/api/posts/search`, { searchParams: { q: query, ...makeSearchParams(params) } }).json()
}

export function addPost(post: PostInput): Promise<Post> {
  return ky.post("/api/posts/add", { json: post }).json()
}

export function putPost(id: number, post: PostInput): Promise<Post> {
